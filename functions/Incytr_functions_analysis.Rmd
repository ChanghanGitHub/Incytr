---
title: "Incytr_functions_analysis"
author: "Changhan"
date: "2024-03-30"
...
#' Calculate the average expression level of the selected cluster and apply the cutoff_percentile
...
#' Inferring signaling pathways based on the user-select genes & database (between one pair of clusters)
#' This function also removed the pathways with duplicated genes
#' Output: data frame, all possible L-R-EM-T pathways
...
#' Calculate the average expression level in each cell group of condition 1 and 2
#' Output: a list contains two data frames, average gene expression (in each group) in each condition
...
#' Calculate the log2FoldChang & adjlog2FC for different data type (SigWeight/MS data/fold change)
#' @param df a data frame that have data for different conditions (variables: "gene_symbol", "condition1", "condition2")
```{r}
...
#' (1) add expression information; (2) calculate the chat probability (SigWeight)
```{r}
...
#' Calculate the single-cell fold change (log2FoldChange) using DESeq2
...
#' Integration of the proteomics data (not fold change!)
#' Format of the pr.data: "gene_symbol" + "Cell group_pr"
#' Format of the ps.data: "gene_symbol" + "Cell group_ps"
#' Format of the py.data: "gene_symbol" + "Cell group_py"
```{r}
...
#' simplified version for permutation test
```{r}
...
```{r}
# ptest_function <- function(x, ptest.object = object, mean_method = NULL, K = 0.5, N = 2){
#   
#   ptest.object@idents = object@idents[permutation[, x]]
...
#   
#   return(ptest.object@SigProb[ , c(2,3)])
# }
```



#' Calculate the p_value based on the scRNA-seq data fpr each pathway in each condition
